...
export async function HomePage({ rtl = false }: { rtl?: boolean }) {
  ...
  </main><WhatsAppLink message={generalWhatsAppMessage} className="fixed bottom-5 right-5 z-40 grid size-14 place-items-center rounded-full bg-[#16a34a] text-white shadow-xl" aria-label="Contacter Vay Cars sur WhatsApp"><MessageCircle className="size-6" /></WhatsAppLink><div className="fixed bottom-0 left-0 right-0 z-30 border-t bg-white p-3 shadow-lg md:hidden"><a href="#voitures" className="flex items-center justify-center gap-2 rounded-xl bg-[#0b5aa7] py-3 text-sm font-bold text-white"><Clock3 className="size-4" />{rtl ? "اكتشف السيارات" : "Voir les disponibilités"}</a></div><SiteFooter rtl={rtl} settings={settings} /></div>;
}
