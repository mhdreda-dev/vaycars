...
function blobPath(file: File) {
  ...
  return `vehicles/${crypto.randomUUID()}-${slugify(basename).slice(0, 80) || "vehicle"}.${extension}`;
}
...
export function VehicleForm({ categories, initial = emptyVehicle, vehicleId }: {
  ...
  async function uploadFiles(files: FileList | null) {
    ...
    const invalid = selected.find((file) => !acceptedImageTypes.has(file.type) || file.size > maxImageSize);
    if (invalid) {
      setUploadMessage("Utilisez des fichiers JPEG, PNG, WebP ou AVIF de 10 Mo maximum.");
      return;
    }
