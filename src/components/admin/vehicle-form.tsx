...
export function VehicleForm({ categories, initial = emptyVehicle, vehicleId, blobConfigured = false }: {
  ...
  return <form action={submit} className="space-y-6">
    ...
    {message && <p role="alert" className="rounded-xl bg-blue-50 px-4 py-3 text-sm font-medium text-[#0b5aa7]">{message}</p>}<div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Link href="/admin/vehicles" className="inline-flex min-h-12 items-center justify-center rounded-xl border border-slate-200 px-5 text-sm font-bold text-[#10233c]">Annuler</Link><button type="submit" disabled={pending} className="min-h-12 rounded-xl bg-[#0b5aa7] px-5 text-sm font-bold text-white disabled:opacity-60">{pending ? "Enregistrement…" : vehicleId ? "Enregistrer les modifications" : "Ajouter la voiture"}</button></div>
  </form>;
}
