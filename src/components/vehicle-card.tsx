...
export function VehicleCard({ vehicle, rtl = false, priority = false, index = 0 }: { vehicle: PublicVehicle; rtl?: boolean; priority?: boolean; index?: number }) {
  ...
  return <motion.article layout={!reduceMotion} initial={{ opacity: 0, y: reduceMotion ? 0 : 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: reduceMotion ? 0 : -10 }} transition={{ duration: reduceMotion ? 0.2 : 0.42, delay: reduceMotion ? 0 : Math.min(index, 5) * 0.07, ease: "easeOut" }} whileHover={reduceMotion ? {} : { y: -5 }} className="group flex h-full min-h-[430px] w-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_10px_30px_rgba(15,35,60,.06)] transition-colors duration-200 hover:border-blue-200 hover:shadow-[0_18px_42px_rgba(15,35,60,.12)] sm:min-h-[480px]">
