...
const darijaNavItems = [["الرئيسية", "#accueil"], ["الطوموبيلات", "#voitures"], ["كيفاش كاتخدم", "#fonctionnement"], ["علاش Vay Cars", "#avantages"], ["علينا", "#apropos"], ["الأسئلة", "#faq"], ["تاصل بينا", "#contact"]];
...
export function Navigation({ rtl = false, hero = false }: { rtl?: boolean; hero?: boolean }) {
  ...
  const visibleNavItems = rtl ? darijaNavItems : navItems;
  ...
  return <motion.header initial={{ opacity: 0, y: reduceMotion ? 0 : -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: "easeOut" }} className={`sticky top-0 z-50 transition-[background-color,border-color,box-shadow] duration-300 ${compact ? "border-b border-slate-200/80 bg-white/90 shadow-sm backdrop-blur-md" : "border-b border-transparent bg-white/55 backdrop-blur-sm"}`}>
    <nav className={`mx-auto flex max-w-7xl items-center justify-between px-4 transition-[height] duration-300 sm:px-6 lg:px-8 ${compact ? "h-16" : "h-[4.5rem]"}`} aria-label="Navigation principale">
      ...
      <div className="hidden items-center gap-5 xl:flex">{visibleNavItems.map(([label, href]) => <a className="text-sm font-medium text-slate-600 transition hover:text-[#0b5aa7]" href={href} key={label}>{label}</a>)}<Link href={languageHref} onClick={() => saveLocalePreference(targetLocale)} className="rounded-lg px-2 py-1 text-sm font-semibold text-slate-600">{rtl ? "FR" : "العربية"}</Link><WhatsAppLink message={generalWhatsAppMessage} className="flex items-center gap-2 rounded-xl bg-[#16a34a] px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-[#15803d]">{rtl ? "حجز فالواتساب" : "Réserver maintenant"}</WhatsAppLink></div>
    ...
    {open && <div className="border-t bg-white px-5 py-5 shadow-xl xl:hidden"><div className="flex flex-col gap-1">{visibleNavItems.map(([label, href]) => <a onClick={() => setOpen(false)} className="rounded-xl px-4 py-3 text-base font-semibold text-slate-700 hover:bg-slate-50" href={href} key={label}>{label}</a>)}<Link href={languageHref} onClick={() => { saveLocalePreference(targetLocale); setOpen(false); }} className="rounded-xl px-4 py-3 font-semibold text-slate-700">{rtl ? "Français" : "العربية"}</Link><WhatsAppLink message={generalWhatsAppMessage} className="mt-2 flex items-center justify-center gap-2 rounded-xl bg-[#16a34a] px-4 py-3.5 font-bold text-white"><Phone className="size-4" />{rtl ? "حجز فالواتساب" : "Réserver sur WhatsApp"}</WhatsAppLink></div></div>}
  </motion.header>;
}
