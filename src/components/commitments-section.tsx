...
const arabicCommitments = [
  [Info, "معلومات واضحة", "يتم توضيح الشروط المهمة قبل تأكيد الحجز."],
  [MessageCircle, "رد سريع", "تتم معالجة طلبات التوفر والثمن مباشرة عبر واتساب."],
  [BadgeCheck, "سيارات مجهزة", "يتم فحص وتجهيز كل سيارة قبل تسليمها للعميل."],
  [Handshake, "مواكبة شخصية", "تبقى الوكالة متاحة لمساعدتك قبل وأثناء مدة الكراء."],
] as const;

export function CommitmentsSection({ rtl = false }: { rtl?: boolean }) {
  const reduceMotion = useReducedMotion();
  const copy = rtl
    ? { label: "التزاماتنا", title: "خدمة واضحة من أول حجز", description: "تلتزم Vay Cars Location بتقديم معلومات واضحة، وتواصل سريع، وسيارات مجهزة لرحلتك." }
  ...
  return <section className="bg-[#f1f5f8] py-20 sm:py-24" dir={rtl ? "rtl" : "ltr"}>
    <motion.div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.16 }} variants={{ hidden: {}, visible: { transition: { staggerChildren: reduceMotion ? 0 : 0.1 } } }}>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {commitments.map(([Icon, title, description], index) => <motion.article key={title} variants={{ hidden: { opacity: 0, y: reduceMotion ? 0 : 20 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.42, delay: reduceMotion ? 0 : index * 0.03 }} className="flex min-h-[216px] flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_10px_28px_rgba(15,35,60,.06)]">
